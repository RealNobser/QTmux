...
**Stand 2026-08-10: 29 Tickets offen.**
...
| **131** | **„Link kopieren" im Rechtsklick-Menü** | in einer Session z. B. `echo https://example.org/pfad` und `ls` ausführen, dann **Rechtsklick auf die URL**: oben im Kontextmenü muss „Link kopieren" mit Trennlinie stehen, danach liegt die URL in der Zwischenablage (Cmd+V in die Shell). Rechtsklick auf einen **Dateinamen** aus dem `ls`: kopiert wird der **absolute** Pfad. Rechtsklick auf leeren Text: Menü wie bisher, ohne den Eintrag. Gegenprobe: „Rechtsklick fügt ein" in den Einstellungen aktivieren → Rechtsklick fügt weiter ein, kein Menü |
